...
//typedef struct locate {
  ...
  // int row;
//} locate;
char keys1[4][4] = {{0xd,0xf,0x0,0xe}, 
                    {0xc,0x9,0x8,0x7},
                     {0xb,0x6,0x5,0x4},
                     {0xa,0x3,0x2,0x1}};	
char keys[4][4] = {{0x1,0x2,0x3,0xa}, 
...
#define GPIOA_MODER_11to8_OUTPUT 0x0550000
#define GPIOA_MODER_PA0_AF	0x1
#define GPIOB_MODER_6to3_0_OUTPUT  0x00001540
...
void PinSetup(void) {
	...
	/* Set PB[6:3] and 	PB0 as output mode */
	GPIOB->MODER |=  GPIOB_MODER_6to3_0_OUTPUT;
...
void PB0_Interrupt_Pin_Setup(void) {
...
void EXTI0_IRQHandler(void) {
  ...
  locate loc = find_key();
		if (loc.col != -1 && loc.row != -1) {
			key_pressed = keys[loc.row][loc.col];
		}	
	...
	/* 30% */
	else if ( (key_pressed) == 0x3 ) {
		duty = (period * 0.3);
		TIM2->CCR1 = duty;
	//	TIM2->CR1 |= TIM_CR1_CEN; //enable TIM2 to count
	}
	/* 40% */	
	else if ( (key_pressed) == 0x4 ) {
		duty = (period * 0.4);
		TIM2->CCR1 = duty;
	//TIM2->CR1 |= TIM_CR1_CEN; //enable TIM2 to count
	}
	...
	/* 60% */
	else if ( (key_pressed) == 0x6 ) {
		duty = (period * 0.6);
		TIM2->CCR1 = duty;
		//TIM2->CR1 |= TIM_CR1_CEN; //enable TIM2 to count
	}
	/* 70% */	
	else if ( (key_pressed) == 0x7 ) {
		duty = (period * 0.7);
		TIM2->CCR1 = duty;
		//TIM2->CR1 |= TIM_CR1_CEN; //enable TIM2 to count
	}
	/* 80% */
	else if ((key_pressed) == 0x8 ) {
		duty = (period * 0.8);
		TIM2->CCR1 = duty;
	//	TIM2->CR1 |= TIM_CR1_CEN; //enable TIM2 to count
	}
	/* 90% */
	else if ( (key_pressed) == 0x9 ) {
		duty = (period * 0.9);
		TIM2->CCR1 = duty;
		//TIM2->CR1 |= TIM_CR1_CEN; //enable TIM2 to count
	}
	/* 100% */
	else if ( (key_pressed) == 0xA ) {
		duty = (period * 1);
		TIM2->CCR1 = duty;
		//TIM2->CR1 |= TIM_CR1_CEN; //enable TIM2 to count
	}
